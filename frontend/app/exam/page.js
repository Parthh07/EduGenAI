        ...
        {!isExamActive && !isSubmitted && (
          <div className="animate-in fade-in zoom-in-95">
            <header className="text-center mb-12">
              ...
              <p className="text-slate-400 text-sm md:text-base font-medium">Generate a timed, AI-graded practice exam from your documents.</p>
            </header>

            <div className="bg-[#0A0A0A] border border-white/10 p-8 rounded-3xl shadow-2xl">
              {/* File Dropzone */}
              <div className="relative group rounded-3xl border border-dashed border-white/20 hover:border-cyan-500/50 hover:bg-cyan-500/[0.02] bg-[#050505] transition-all p-8 md:p-10 text-center flex flex-col items-center justify-center cursor-pointer mb-8 overflow-hidden min-h-[180px]">
                ...
                {files.length === 0 ? (
                  <>
                    <svg className="w-8 h-8 text-cyan-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
                    <p className="text-lg font-bold text-slate-300 group-hover:text-cyan-400 transition-colors z-0">Select Exam Syllabus</p>
                    ...
                    <div className="grid grid-cols-1 gap-3 text-left max-h-[200px] overflow-y-auto pr-2">
                      {files.map((file, idx) => (
                        <div key={idx} className="flex items-center gap-3 bg-[#0A0A0A] border border-white/10 p-4 rounded-2xl shadow-lg hover:border-cyan-500/50 transition-colors group/file relative z-30">
                          <svg className="w-6 h-6 text-cyan-400 flex-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                          ...
                          <button onClick={(e) => { e.preventDefault(); const nf = files.filter((_, i) => i !== idx); setFiles(nf); setGlobalFiles(nf); }} className="text-slate-500 hover:text-red-400 p-2 rounded-xl hover:bg-red-500/10 transition-all opacity-0 group-hover/file:opacity-100 cursor-pointer">
              ...
              <div className="grid md:grid-cols-4 gap-4 mb-8">
                <div>
                  ...
                  <select value={examType} onChange={(e) => setExamType(e.target.value)} className="bg-[#050505] border border-white/10 rounded-xl p-3.5 w-full outline-none focus:ring-1 focus:ring-cyan-500 text-sm">
                    <option value="MCQ">Multiple Choice</option>
                    <option value="Theory">Theory / Essay</option>
                  </select>
                </div>
                <div>
                  <label className="block text-slate-400 mb-2 text-xs font-semibold uppercase tracking-widest">Difficulty</label>
                  <select value={difficulty} onChange={(e) => setDifficulty(e.target.value)} className="bg-[#050505] border border-white/10 rounded-xl p-3.5 w-full outline-none focus:ring-1 focus:ring-cyan-500 text-sm">
                ...
                <div>
                  ...
                  <select value={qCount} onChange={(e) => setQCount(e.target.value)} className="bg-[#050505] border border-white/10 rounded-xl p-3.5 w-full outline-none focus:ring-1 focus:ring-cyan-500 text-sm">
                ...
                <div>
                  <label className="block text-slate-400 mb-2 text-xs font-semibold uppercase tracking-widest">Time (mins)</label>
                  <input type="number" min="1" max="180" value={timerMins} onChange={(e) => setTimerMins(e.target.value)} className="bg-[#050505] border border-white/10 rounded-xl p-3.5 w-full outline-none focus:ring-1 focus:ring-cyan-500 text-sm" />
                </div>
              </div>
              ...
              <button onClick={handleStartExam} disabled={loading} className="w-full bg-white text-black hover:bg-slate-200 py-3.5 flex justify-center items-center gap-2 rounded-xl font-bold transition-all disabled:opacity-50">
                {loading ? <span className="w-5 h-5 border-2 border-black/30 border-t-black rounded-full animate-spin" /> : "Begin Assessment"}
              </button>
            </div>
          </div>
        )}
              ...
              {questions.map((q, idx) => (
                <div key={idx} className="bg-[#0A0A0A] border border-white/10 p-6 md:p-8 rounded-3xl shadow-lg">
                  ...
                  {examType === 'MCQ' ? (
                    ...
                    <textarea rows="6" placeholder="Write your answer here..." value={answers[idx] || ''} onChange={(e) => handleAnswerChange(idx, e.target.value)} className="w-full bg-[#050505] border border-white/10 rounded-xl p-4 text-slate-200 text-sm outline-none focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 resize-y" />
                  )}
                </div>
              ))}
            </div>
        ...
        {isSubmitted && (
          <div className="animate-in fade-in zoom-in-95 mt-8">
            <div className="text-center mb-10">
              <h2 className="text-3xl font-bold tracking-tight text-white mb-6">Assessment Result</h2>

              {gradingLoading ? (
                <div className="inline-flex flex-col items-center gap-4 bg-[#0A0A0A] border border-white/10 rounded-3xl p-10 shadow-2xl">
                ...
                <div className="inline-block bg-[#0A0A0A] border border-white/10 rounded-3xl p-8 shadow-2xl min-w-[200px]">
                  ...
                  <p className="text-5xl font-black text-white">{score}<span className="text-2xl text-slate-500">%</span></p>
                ...
                <div className="inline-block bg-[#0A0A0A] border border-white/10 rounded-3xl p-8 shadow-2xl">
                  <p className="text-slate-300 text-sm font-medium">Exam submitted. Review your answers below.</p>
                </div>
              )}
            </div>
            ...
            {!gradingLoading && (
              <div className="space-y-6">
                <h3 className="text-sm tracking-widest uppercase text-slate-500 font-bold border-b border-white/10 pb-4 mb-2">Question Review</h3>
                {questions.map((q, idx) => {
                      ...
                      {examType === 'MCQ' ? (
                        <div className="grid md:grid-cols-2 gap-4">
                          <div className="bg-[#050505] p-4 rounded-xl border border-white/5">
                            ...
                            <p className={`text-sm font-medium ${isCorrect ? 'text-emerald-400' : 'text-red-400'}`}>{answers[idx] || "Omitted"}</p>
                          </div>
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center gap-3 mb-4">
                            ...
                            <div className="flex-1 h-2 bg-[#050505] rounded-full overflow-hidden border border-white/5">
                          ...
                          <div className="grid md:grid-cols-2 gap-4">
                            <div className="bg-[#050505] p-4 rounded-xl border border-white/5">
                              <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-2">Your Answer</p>
                              <p className="text-slate-300 text-sm leading-relaxed">{answers[idx] || "No answer provided"}</p>
                            </div>
                            <div className="bg-[#050505] p-4 rounded-xl border border-white/5">
                          ...
                          {grade.modelAnswer && (
                            <div className="bg-emerald-500/5 p-4 rounded-xl border border-emerald-500/10">
                              <p className="text-emerald-400 text-[10px] font-bold uppercase tracking-widest mb-2">Model Answer</p>
                              <p className="text-slate-300 text-sm leading-relaxed">{grade.modelAnswer}</p>
                            </div>
                          )}
                        </div>
                      ) : (
            ...
            <div className="no-print mt-12 text-center flex flex-wrap justify-center gap-3">
              <button onClick={() => window.print()} className="bg-[#0A0A0A] hover:bg-white/5 text-slate-300 font-bold px-6 py-3 rounded-xl transition-all border border-white/10 text-sm">Print Report</button>
              <button onClick={() => { setIsSubmitted(false); setQuestions(null); setFiles([]); setGlobalFiles([]); setTheoryGrades(null); }} className="bg-white hover:bg-slate-200 text-black font-bold px-6 py-3 rounded-xl transition-all text-sm shadow-md">New Assessment</button>
            </div>
          </div>
        )}
      </main>
